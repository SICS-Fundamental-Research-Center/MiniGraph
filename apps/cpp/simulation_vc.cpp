...
class SimulationPIE : public minigraph::AutoAppBase<GRAPH_T, CONTEXT_T> {
 ...
 public:
  ...
  bool PEval(GRAPH_T& graph,
    ...
    this->auto_map_->ActiveMap(
        graph, task_runner, &visited,
        SimulationAutoMap<GRAPH_T, CONTEXT_T>::kernel_push_border_vertexes,
  ...
  bool IncEval(GRAPH_T& graph,
    ...
    this->auto_map_->ActiveMap(
        graph, task_runner, &visited,
        SimulationAutoMap<GRAPH_T, CONTEXT_T>::kernel_pull_border_vertexes,
    ...
    delete in_visited;
    delete out_visited;
    if (global_border_vid_map->get_bit(this->context_.root_id)) {
      auto uu = graph.GetVertexByVid(this->context_.root_id);
      uu.ShowVertexInfo();
    }
...
int main(int argc, char* argv[]) {
  ...
  auto app_wrapper =
      new minigraph::AppWrapper<SimulationPIE<CSR_T, Context>, CSR_T>(bfs_pie);
