...
class CSRIOAdapter : public IOAdapterBase<GID_T, VID_T, VDATA_T, EDATA_T> {
 ...
 public:
  ...
  bool Read(graphs::Graph<GID_T, VID_T, VDATA_T, EDATA_T> *graph,
    ...
    std::string pt[] = {(args)...};
    switch (graph_format) {
      case edgelist_csv:return this->ReadCSRFromEdgeListCSV(graph, pt[0]);
      case csr_bin:return this->ReadCSRFromCSRBin(graph, gid, pt[0], pt[1], pt[2]);
